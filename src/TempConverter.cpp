...
void TempConverter::SetTemp(float val, char unit){
    switch(unit){
        case 'F':
        case 'f':
            TempConverter::tempF = val;
            TempConverter::tempC = TempF2C(val);
            break;
        case 'C':
        case 'c':
            TempConverter::tempC = val;
            TempConverter::tempF = TempC2F(val);
            break;
        case 'K':
        case 'k':
...
float TempConverter::GetF(){
    return TempConverter::tempF;
}

float TempConverter::GetC(){
    return TempConverter::tempC;
}
